...
class viewtablewater extends StatefulWidget {
  // final list;
  // final i;
  // final x;
  String mon_Start;
  ...
  viewtablewater(String mon_Start, mon_End, yearSave) {
  ...
  //   return _viewtablewaterState(mon_Start);
  // }
  _viewtablewaterState createState() =>
      _viewtablewaterState(mon_Start, mon_End, yearSave);
}

class _viewtablewaterState extends State<viewtablewater> {
  String mon_Start;
  String mon_End;

  String yearSave;
  _viewtablewaterState(String mon_Start, mon_End, yearSave) {
    this.mon_Start = mon_Start;
    this.mon_End = mon_End;
  ...
  Future<List> getDataToTable() async {
    final response = await http.get(Uri.parse(
        'http://$ipcon/greenhousegas/show_water_table.php?water_year=${yearSave}&mon_Start=${mon_Start}&mon_End=${mon_End}'));
    //print(response.body);
                            ...
                            for (var item in dataTable)
                              TableRow(children: [
                                ...
                                Padding(
                                  padding: const EdgeInsets.only(
                                      right: 0, top: 0, bottom: 10, left: 20),
                                  child: Text(item['water_cubic_eq'],
                                      style: TextStyle(
                                          fontSize: 14,
                                          color: Colors.grey[700])),
                                ),
